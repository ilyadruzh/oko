...
use crate::scan::bitcoin::blockchain::proto::script::custom::eval_from_bytes_custom;
...
pub fn eval_from_bytes_bitcoin(bytes: &[u8], version_id: u8) -> EvaluatedScript {
    ...
    let address = match Address::from_script(script, network) {
        ...
        Err(err) => {
            if err != address::FromScriptError::UnrecognizedScript {
...
fn p2pk_to_string(script: &Script, network: Network) -> Option<String> {
    ...
    let address = Address::p2pkh(pkh, network); // Payload::PubkeyHash(pkh),
