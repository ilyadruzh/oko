...
use crate::scan::bitcoin::blockchain::proto::script::{EvaluatedScript, ScriptError, ScriptPattern};
...
impl<'a> ScriptEvaluator<'a> {
    ...
    fn maybe_push_data(&mut self, opcode: Opcode, opcode_class: Class) -> Result<usize, ScriptError> {
    ...
    fn eval_script_pattern(elements: &[StackElement]) -> ScriptPattern {
        ...
        // 65 BYTES:... CHECKSIG
        let gen_block_pattern = [StackElement::Op(opcodes::Opcode::OP_CHECKSIG)];
